...
func (ce *ConversationExecutor) Execute(conversationID uint, userInput string, messageType models.MessageType, conversationState *ConversationState) (string, error) {
	packages, err := ce.getPackageListFromCache()
	if err != nil {
		log.Printf("Error getting package list: %v", err)
		return "", err
	}
	messages := ce.prepareMessages(conversationState.ConversationHistory, packages, userInput)
	...
	for {
		...
		botResponse, err = ce.processInput(conversationID, userInput, conversationState)
		if err != nil {
			return "", err
		}
	}
...
func (ce *ConversationExecutor) processInput(conversationID uint, userInput string, conversationState *ConversationState) (string, error) {
	var botResponse string
...
func (ce *ConversationExecutor) prepareMessages(history []openai.ChatCompletionMessage, packages []external.Package, userInput string) []openai.ChatCompletionMessage {
